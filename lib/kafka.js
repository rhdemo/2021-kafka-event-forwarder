...
if (KAFKA_SVC_USERNAME) {
  log.info(
    'KAFKA_SVC_USERNAME was set in environment. Using SASL and SSL to connect to Kafka.'
  );
  ssl = {
...
module.exports = () => {
  return new Promise((resolve) => {
    ...
    const funcs = {
       ...
       */
      forwardMatchStartUpdate: _sender('start'),
       ...
       * @param {MatchInstance} instance
       */
      forwardMatchEndUpdate: _sender('end'),
       ...
       */
      forwardMatchAttackUpdate: _sender('attack')
    };
    ...
    function _sender(type) {
      return async (data) => {
        const message = {
          ...
          value: JSON.stringify({ type, data, cluster })
        };
